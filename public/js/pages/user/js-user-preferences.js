...
createSearchBox.addSearchButton(searchBox,searchBoxHtml,"preferences-content","list-group-item");

var xhr = new XMLHttpRequest(); 
xhr.open("POST",new TaskUrls().concatBaseUrl("/user/preference/get-preference-list"),true); 
	...
	if (xhr.readyState != 4) { return }
		if (xhr.status !=200) {
			...
			Object.keys(result).forEach((key)=>{
				var item = result[key];
				var contentCapsul = {"tag":"a","class":"list-group-item mt-3",child:[
				...
				switch(item["set_form_type"]){
					...
					case "password":
						contentCapsul.child[2].child.push(
							{"tag":"div","class":"form-input","child":
							[
								{"tag":"button","id":"password","class":"form-control btn-primary","name":"password","text":"Yeni Parola Belirle"}, 
							]
						});
					break;
					default:
			...
			taskElements.createElements(document.getElementById("preferences-content"),bodyHtml);
		}
	} 
/*
